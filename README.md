...
| `count` | Partial | Counts documents matching the supported filter subset, with `skip` and `limit`; supports explicit command `hint` for safe exact/prefix/range index plans and `explain: true` diagnostics. PyMongo `estimated_document_count()` uses this path. | No collation, read concern, maxTimeMS, or storage-stat semantics. PyMongo `count_documents()` uses aggregate and does not get command-level count hints. |
...
| `find` | Partial | Returns `firstBatch` and creates a per-client server-side cursor when more shaped results remain. Supports exact matches, dotted paths, limited array traversal, `$eq`, `$ne`, `$gt`, `$gte`, `$lt`, `$lte`, `$in`, `$nin`, `$exists`, `$regex`, `$type`, `$size`, `$all`, `$elemMatch`, `$and`, `$or`, `$nor`, `$not`, projection, sort, skip, limit, capped batch size, `hint`, `explain: true`, conservative exact/prefix/range index narrowing, and sort-aware reads for unique fully covered bool/ObjectId/date scalar index keys. | No `$where`, geospatial/text search, collation, read concern, tailable cursors, text/geospatial/hashed/wildcard index planning, or broad sort pushdown. Unsupported operators and unsupported predicate shapes return command errors. |
| `findAndModify` / `findandmodify` | Partial | Supports PyMongo `find_one_and_update`, `find_one_and_replace`, and `find_one_and_delete` for one document, with filter, deterministic sort, `fields`/`projection`, pre-image or post-image return, update/replacement upsert, supported update modifiers, `_id` immutability, validator enforcement, `bypassDocumentValidation: true`, unique-index enforcement, maintained index entries, and supported `hint`. | No array filters, pipeline updates, positional updates, collation, write concern, maxTimeMS, `let`, retryable writes, transaction semantics, or explain behavior. Unsupported shapes return command errors. |
...
| Indexes | Partial | Supports `createIndexes`, `listIndexes`, and `dropIndexes` metadata for simple ascending/descending single-field and compound keys, lists virtual `_id_`, enforces supported `unique: true` indexes across insert/update/upsert, maintains safe exact, compound-prefix, range, scalar multikey, sparse, and partial planner entries, accepts supported hints, and exposes partial explain diagnostics. | Text, geospatial, hashed, wildcard, TTL, hidden, collation, background semantics, full compound multikey planning, numeric range planning, string sort pushdown, and full MongoDB explain parity are unsupported. Unique indexes reject array/multikey values. |
