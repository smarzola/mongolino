...
| BSON storage | Partial | Stores BSON blobs in SQLite, derives a stable primary key from `_id`, maintains scalar equality index entries for supported simple indexes, and stores durable collection validator metadata. Inserts with operator-shaped field names store those names as data. | Validator enforcement is being added; no document size enforcement beyond message size, compound index planning, or range planning. |
