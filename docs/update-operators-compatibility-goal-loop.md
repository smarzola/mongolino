...
- [x] Milestone 3: Invariant hardening across validation, indexes, and findAndModify
- [x] Milestone 4: PyMongo/spec corpus, README, and final verification

Milestone 0 status, 2026-07-04: refactored modifier parsing into structured storage and centralized update path validation while keeping existing `$set`, `$unset`, and `$inc` behavior unchanged. Verification passed: `cargo fmt -- --check`; `cargo test update`; `cargo test find_and_modify`; `cargo test validation`; `cargo test`; `cargo build`. Commit: `6697578`.

Milestone 1 status, 2026-07-04: implemented `$rename`, `$min`, `$max`, `$mul`, and `$setOnInsert` through shared `update` and `findAndModify` update application, with Rust and PyMongo coverage plus scalar spec-corpus cases. Verification passed: `cargo fmt -- --check`; `cargo test update`; `cargo test find_and_modify`; `cargo test validation`; `cargo test`. Verification blocked by sandbox localhost permissions: `UV_CACHE_DIR=/private/tmp/mongolino-uv-cache uv run --locked pytest tests/e2e/test_update_operators.py` failed during fixture setup with `PermissionError: [Errno 1] Operation not permitted` on `sock.bind(("127.0.0.1", 0))`. Commit: `592f5b0`.

Milestone 2 status, 2026-07-04: implemented `$push` with scalar and `$each`, `$addToSet` with scalar and `$each`, `$pop`, `$pull` equality plus existing matcher predicates, and `$pullAll`, while keeping `$position`, `$slice`, `$sort`, positional paths, scalar parents, and malformed operands as explicit write errors. Verification passed: `cargo fmt -- --check`; `cargo test update`; `cargo test`. Verification blocked by sandbox localhost permissions: `UV_CACHE_DIR=/private/tmp/mongolino-uv-cache uv run --locked pytest tests/e2e/test_update_operators.py` failed during fixture setup with `PermissionError: [Errno 1] Operation not permitted` on `sock.bind(("127.0.0.1", 0))`. Commit: `bd164a3`.

Milestone 3 status, 2026-07-04: added focused invariant hardening for new modifiers across validation failures and bypass, unique-index duplicate/array rejection, maintained index-entry freshness, `findAndModify` pre/post images, and ordered/unordered batch semantics. Verification passed: `cargo fmt -- --check`; `cargo test update`; `cargo test unique`; `cargo test validation`; `cargo test planner`; `cargo test`. Verification blocked by sandbox localhost permissions: `UV_CACHE_DIR=/private/tmp/mongolino-uv-cache uv run --locked pytest tests/e2e/test_update_operators.py tests/e2e/test_errors.py` failed during fixture setup with `PermissionError: [Errno 1] Operation not permitted` on `sock.bind(("127.0.0.1", 0))`. Commit: `d5b5e13`.

Milestone 4 status, 2026-07-04: updated README compatibility notes and spec corpus coverage for the supported scalar and array update subset, unsupported update options, `findAndModify`, and invariant cases. Verification passed: `cargo fmt -- --check`; `cargo test`; `cargo build`; `UV_CACHE_DIR=/private/tmp/mongolino-uv-cache uv lock --check`; `UV_CACHE_DIR=/private/tmp/mongolino-uv-cache uv sync --locked --dev`. Verification blocked by sandbox localhost permissions: `UV_CACHE_DIR=/private/tmp/mongolino-uv-cache uv run --locked pytest tests/e2e` collected 113 tests but failed server-backed tests with `PermissionError: [Errno 1] Operation not permitted` on `sock.bind(("127.0.0.1", 0))`; non-server setup tests reported 6 passed and 1 skipped before the bind-blocked failures. Commit hash reported after commit creation.
