...
- [x] Milestone 2: Cursor lifecycle hardening and `killCursors`
  - Status 2026-07-04: Implemented `killCursors`, killed/not-found response arrays, malformed cursor-id errors, namespace mismatch handling, `getMore` after kill/exhaustion errors, PyMongo cursor lifecycle tests, local corpus killCursors coverage, and README updates. Verification: `cargo fmt -- --check` passed; `cargo test cursor` passed (4 tests); `cargo test kill` passed (4 tests); `cargo test` passed (47 tests); `cargo build` passed with dead-code warnings for test wrappers; unsandboxed `UV_CACHE_DIR=/private/tmp/mongolino-uv-cache uv run --locked pytest tests/e2e` passed (47 passed, 1 skipped). Commit hash reported after commit.
- [x] Milestone 3: Durable collection catalog and lifecycle commands
  - Status 2026-07-04: Added durable `collections` catalog, document-namespace surfacing, catalog creation on writes, `create`, `listCollections`, `drop`, and `dropDatabase`, PyMongo lifecycle tests, local lifecycle corpus cases, and README lifecycle rows. Verification: `cargo fmt -- --check` passed; `cargo test catalog` passed (3 tests); `cargo test drop` passed (2 tests); `cargo test` passed (52 tests); `cargo build` passed with dead-code warnings for test wrappers; unsandboxed `UV_CACHE_DIR=/private/tmp/mongolino-uv-cache uv run --locked pytest tests/e2e` passed (55 passed, 1 skipped). Commit hash reported after commit.
- [x] Milestone 4: Count, distinct, and metadata commands
  - Status 2026-07-04: Added `count`, `distinct`, and a narrow `aggregate` count-documents path for PyMongo, including skip/limit count handling, scalar/dotted/array distinct values, unsupported option errors, PyMongo metadata tests, local count/distinct corpus coverage, and README metadata rows. Verification: `cargo fmt -- --check` passed; `cargo test count` passed (5 tests); `cargo test distinct` passed (2 tests); `cargo test` passed (56 tests); `cargo build` passed with dead-code warnings for test wrappers; unsandboxed `UV_CACHE_DIR=/private/tmp/mongolino-uv-cache uv run --locked pytest tests/e2e` passed (60 passed, 1 skipped). Commit hash reported after commit.
- [x] Milestone 5: Index catalog and index command compatibility
  - Status 2026-07-04: Added durable index catalog metadata, virtual `_id_` listing, `createIndexes`, `listIndexes`, `dropIndexes`, simple key validation, unique metadata storage, unsupported option errors, PyMongo index tests, local index corpus coverage, and README index notes. Verification: `cargo fmt -- --check` passed; `cargo test index` passed (3 tests); `cargo test` passed (59 tests); `cargo build` passed with dead-code warnings for test wrappers; unsandboxed `UV_CACHE_DIR=/private/tmp/mongolino-uv-cache uv run --locked pytest tests/e2e` passed (65 passed, 1 skipped). Commit hash reported after commit.
- [x] Milestone 6: Unique index enforcement
  - Status 2026-07-04: Enforced supported unique indexes during index creation, insert, replacement/modifier update, and upsert; preserved ordered/unordered bulk behavior; rejected array/multikey unique values; removed enforcement after index drop; added Rust, PyMongo, and corpus coverage; updated README unique-index notes. Verification: `cargo fmt -- --check` passed; `cargo test unique` passed (4 tests); `cargo test index` passed (7 tests); `cargo test` passed (63 tests); `cargo build` passed with dead-code warnings for test wrappers; unsandboxed `UV_CACHE_DIR=/private/tmp/mongolino-uv-cache uv run --locked pytest tests/e2e` passed (70 passed, 1 skipped). Commit hash reported after commit.
- [x] Milestone 7: Simple index-aware query execution
  - Status 2026-07-04: Added durable scalar equality `index_entries`, maintained entries on create/insert/update/upsert/delete/drop, used single-field index entries as conservative `find` candidates before matcher validation, added Rust planner freshness tests, PyMongo indexed mutation tests, corpus coverage, and README planner notes. Verification: `cargo fmt -- --check` passed; `cargo test planner` passed (2 tests); `cargo test index` passed (8 tests); `cargo test` passed (65 tests); `cargo build` passed with dead-code warnings for test wrappers; unsandboxed `UV_CACHE_DIR=/private/tmp/mongolino-uv-cache uv run --locked pytest tests/e2e` passed (72 passed, 1 skipped). Commit hash reported after commit.
- [x] Milestone 8: PyMongo/spec corpus expansion and docs
  - Status 2026-07-04: Completed final coverage/docs pass, added targeted e2e README notes, removed build-only dead-code warnings for test wrappers, and ran final verification. Verification: `cargo fmt -- --check` passed; `cargo test` passed (65 tests); `cargo build` passed; `UV_CACHE_DIR=/private/tmp/mongolino-uv-cache uv lock --check` passed; `UV_CACHE_DIR=/private/tmp/mongolino-uv-cache uv sync --locked --dev` passed; unsandboxed `UV_CACHE_DIR=/private/tmp/mongolino-uv-cache uv run --locked pytest tests/e2e` passed (72 passed, 1 skipped). Commit hash reported after commit.
